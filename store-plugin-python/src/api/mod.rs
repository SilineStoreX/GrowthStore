...
use crate::engine::python_plugin::PythonPluginConfig;
...
pub async fn get_schema(
    ...
    let uri = format!("python://{ns}/{name}#{method}");
            ...
            None => {
                resp.status_code(StatusCode::NOT_FOUND)
                    .render(Json(ApiResult::<String>::error(
                        400,
                        &format!("python service {invoke_uri:?} was not found"),
                    )));
            }
        }
    } else {
        ...
        let resdata = match MxStoreService::get_plugin_service(&invoke_uri.url_no_method()) {
            Some(pls) => {
                ...
                if let Some(val) = pls.get_config() {
                    if let Ok(tconf) = serde_json::from_value::<PythonPluginConfig>(val) {
