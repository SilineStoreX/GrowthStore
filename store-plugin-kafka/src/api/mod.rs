...
// 使用定义的Kafka连接来发送Kafka消息
// Kafka的发送是一个标准的协义，发送过程中，无法太多的配置
// 如此，我们通过Kafka的send方法来来发送时，主要是使用topic，以及payload来作为参数即可。
// 因此，我们定义了发送接口如下：
...
#[handler]
pub async fn publish_kafka_request(
    depot: &mut Depot,
    req: &mut Request,
    ...
    let uri = format!("kafka://{ns}/{name}#{method}");
    let mut args = vec![];
